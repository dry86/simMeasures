...
def plot_data_from_excel(dir_path, task_suffix, category, measure_columns, color_palette, mark, save_path):
    ...
    # 删除空项并对每组sheet按字母排序
    data_groups = {key: sorted(value, key=lambda x: x[0].lower()) for key, value in data_groups.items() if value}
    ...
    for group, sheets in data_groups.items():
        ...
        fig.suptitle(f"{category} ({group.capitalize()})", fontweight='bold', fontsize=16)
        ...
        for idx, (file, sheet_name) in enumerate(sheets):
            ...
            # 设置标题、标签和图例
            ax.set_title(f"{replace_keys_with_values(file)}", fontweight='bold', fontsize=12)   # Todo: 调整这里 设置subtitle
        ...
        plt.savefig(f"{save_dir_cate}/{category}_lineplots_{group}.png", dpi=300, bbox_inches='tight')  # non_homogeneous_models_
        # plt.show()
        plt.close(fig)
...
if __name__ == "__main__":
    ...
    # 分析数据的文件路径
    dir_path = "/newdisk/public/wws/simMeasures/results/final_strategy_tasks_aggre" # /non_homogeneous_models
    ...
    # tasks = ["codeLlama-7b-dsc-7b-base-v1.5", "codeLlama-7b-Qwen2.5-Coder-7B", "dsc-7b-base-v1.5-Qwen2.5-Coder-7B"]
    ...
    # tasks = ["codeLlama-7b-codeLlama-7b-Python"]
