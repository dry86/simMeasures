...
def read_vl_prompts(data_path):
    prompts = []
    image_pre = "/newdisk/public/wws/00-Dataset-AIGC/FHM_new/"
    with jsonlines.open(data_path) as reader:
        for obj in reader:
            # 每行为{"id":29384,"img":"img\/29384.png","text":"they're in a row"}
            image_path = image_pre + obj["img"]
            text = obj["text"]
...
def process_and_save_vl_hidden_states(model, processor, prompts, save_dir, device, batch_size=1, save_every=1000):
    ...
    for i, prompt in enumerate(prompts):
        ...
        last_token_hidden_states = [
            layer_output[0, last_non_padding_index, :].squeeze().cpu()
...
def main(config_path):
    ...
    for config in configs:
        ...
        task = config.get("task", "vl")  # 默认任务名为 vl
        lang = config.get("lang", "en")  # 默认语言为 en
