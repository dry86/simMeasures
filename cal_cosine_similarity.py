...
## 计算cosine_similarity
def calculate_cos(matrix1, matrix2):
    # print(f"matrix1:\n {matrix1}")
    # print(f"matrix2:\n {matrix2}")
...
with jsonlines.open(file_path) as reader:
    for obj in reader:
        ...
        for i in range(num_layers):
            score = calculate_cos(hidden_states_model1[i].reshape(-1, hidden_states_model1[i].shape[-1]),
                                hidden_states_model2[i].reshape(-1, hidden_states_model2[i].shape[-1]))
            print(f"Layer {i} Cosine_Similarity: {score}")
