...
def second_order_cosine_similarity(
    ...
    for i, union_nns_i in enumerate(union_nns):
        u = dists_R[i, union_nns_i]
        v = dists_Rp[i, union_nns_i]

        norm_u = np.linalg.norm(u)
        norm_v = np.linalg.norm(v)

        # 如果范数为零，则设定默认相似度为 0 或 1
        if norm_u == 0 and norm_v == 0:
            cosine_similarities.append(1.0)  # 两个都是零向量，可以认为完全相似
        elif norm_u == 0 or norm_v == 0:
            cosine_similarities.append(0.0)  # 一个为零向量，设为不相似
        else:
            cosine_similarities.append(1 - scipy.spatial.distance.cosine(u, v))
