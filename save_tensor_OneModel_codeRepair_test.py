...
def main(model1_path, model_idx, padding_len, device1, batch_size=1):
    ...
    # Please fix the bug in the following code and generate only the correct code: 
    prompt_ = "Please fix the bug in the following code: " # "Please fix the bug in the following code: " "Please fix the following buggy code to the correct code:"
    
    _prompt = "Do not write explanations or assertions; simply provide only the code. "
    # 打开文件并读取每一行内容
    ...
    for i, line in enumerate(lines, start = 1):
        ...
        if len(prompts) == batch_size:  # 分批加载
            ...
            # generated_texts = tokenizer1.decode(generated_outputs.sequences, skip_special_tokens=True)

            # print("model.generated() texts:")
            # # 打印生成的文本
